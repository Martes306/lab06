...
public final class UseArithmeticService {
    ...
    private static void retrySendOnNetworkError(final NetworkComponent server, final String message) {
         ...
         * until it succeeds.
         */
        boolean controlSendMessage = false;
        while (!controlSendMessage) {
            try {
                server.sendData(message);
                controlSendMessage = true;
            } catch (IOException e) {
                System.out.println("Non sono riuscito a mandare i dati" + e);
            }
        }
    }
    ...
    private static String retryReceiveOnNetworkError(final NetworkComponent server) {
        ...
        while (!controlSendMessage) {
            try {
                ...
                System.out.println("Non sono riuscito a ricevere i dati" + e);
            }
        }
        return response;
    }
